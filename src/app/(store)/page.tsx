...
const SHOWCASE_SLUGS = ["laptops", "audio", "monitores", "perifericos"]

async function getShowcaseData() {
  const categories = await withRetry(() =>
    prisma.category.findMany({
      where: { isActive: true, slug: { in: SHOWCASE_SLUGS } },
      orderBy: { sortOrder: "asc" },
    })
  )

  const showcaseSections = await Promise.all(
  ...
  return showcaseSections.filter((s): s is { category: typeof categories[number]; product: NonNullable<typeof s.product> } => s.product !== null)
}
...
export default async function StoreHome() {
  const [showcaseSections, config] = await Promise.all([
    getShowcaseData(),
    getConfig(),
  ])
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative mx-auto w-full max-w-7xl px-4 py-6 md:py-10">
          <div className="grid items-center gap-8 md:grid-cols-2 md:gap-12">
            <div className="animate-slide-up">
              ...
              <h1
                className="mt-4 text-4xl font-bold tracking-tight md:text-5xl lg:text-6xl text-balance"
              ...
              <p
                className="mt-2 max-w-lg text-base leading-relaxed md:text-lg"
              ...
              <div className="mt-5 flex items-center gap-6 text-sm" style={{ color: "oklch(0.56 0.01 260)" }}>
      ...
      <Reveal variant="up" as="section" className="mx-auto max-w-7xl px-4 -mt-8 relative z-10 pb-10">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
          {benefits.map((benefit, i) => (
            <Reveal key={benefit.title} variant="up" delay={i * 0.1} threshold={0.3}>
              <GlassPanel className="flex items-center gap-3 p-3 md:p-4">
                <div
      ...
      {showcaseSections.map(({ category, product }, i) => (
        <Reveal key={category.slug} variant="up" threshold={0.1}>
          <ProductShowcase
            product={{
              name: product.name,
              slug: product.slug,
            ...
            description={category.description ?? `Descubre nuestra colección de ${category.name.toLowerCase()} premium.`}
            index={i}
          />
        </Reveal>
      ))}
      ...
      >
        ...
        <div className="relative mx-auto max-w-2xl px-4 text-center">
          <GlassPanel variant="strong" className="mx-auto max-w-lg px-8 py-8 md:px-12 md:py-10">
            <h2 className="text-2xl font-bold tracking-tight md:text-3xl" style={{ color: "oklch(0.13 0.01 260)" }}>
              ¿Listo para comprar?
            </h2>
            <p className="mt-2 text-sm leading-relaxed md:text-base" style={{ color: "oklch(0.45 0.01 260)" }}>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-5 inline-flex items-center gap-2 rounded-xl px-8 py-3 font-semibold text-white transition-all active:scale-[0.97]"
              style={{
